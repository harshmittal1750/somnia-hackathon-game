...
describe("SpaceDefender", function () {
  let SpaceDefender;
  let spaceDefender;
  let owner;
  let player1;
  ...
  beforeEach(async function () {
    ...
    SpaceDefender = await ethers.getContractFactory("SpaceDefender");
    spaceDefender = await SpaceDefender.deploy();
    await spaceDefender.deployed();
  });

  it("Should set the right owner", async function () {
    expect(await spaceDefender.owner()).to.equal(owner.address);
  });

  it("Should start with game active", async function () {
    expect(await spaceDefender.gameActive()).to.be.true;
  });

  it("Should allow owner to toggle game active status", async function () {
    expect(await spaceDefender.gameActive()).to.be.true;
    await spaceDefender.connect(owner).toggleGameActive();
    expect(await spaceDefender.gameActive()).to.be.false;
  });

  it("Should not allow non-owner to toggle game active status", async function () {
    await expect(
      spaceDefender.connect(player1).toggleGameActive()
    ).to.be.revertedWith("Only owner can call this function");
  });

  it("Should allow players to submit scores", async function () {
    await spaceDefender.connect(player1).submitScore(1000, 3, 20, "normal");

    const playerStats = await spaceDefender.getPlayerStats(player1.address);
  ...
  it("Should update player's highest score", async function () {
    await spaceDefender.connect(player1).submitScore(1000, 3, 20, "normal");
    await spaceDefender.connect(player1).submitScore(2000, 5, 40, "normal");
  ...
  it("Should not submit score if game is inactive", async function () {
    await spaceDefender.connect(owner).toggleGameActive();
    await expect(
      spaceDefender.connect(player1).submitScore(1000, 3, 20, "normal")
    ).to.be.revertedWith("Game is not active");
  });

  it("Should not submit score if score is zero", async function () {
    await expect(
      spaceDefender.connect(player1).submitScore(0, 3, 20, "normal")
    ).to.be.revertedWith("Score must be greater than 0");
  });

  it("Should not submit score if level is invalid", async function () {
    await expect(
      spaceDefender.connect(player1).submitScore(1000, 0, 20, "normal")
    ).to.be.revertedWith("Invalid level");

    await expect(
      spaceDefender.connect(player1).submitScore(1000, 11, 20, "normal")
    ).to.be.revertedWith("Invalid level");
  });

  it("Should return top scores correctly", async function () {
    await spaceDefender.connect(player1).submitScore(1000, 3, 20, "normal");
    await spaceDefender.connect(player2).submitScore(1500, 4, 30, "normal");
    await spaceDefender.connect(owner).submitScore(800, 2, 15, "normal");

    const topScores = await spaceDefender.getTopScores(3);
  ...
  it("Should emit NewHighScore event for new personal bests", async function () {
    await expect(
      ...
      .to.emit(spaceDefender, "NewHighScore")
      .withArgs(player1.address, 1000);

    await expect(
      spaceDefender.connect(player1).submitScore(2000, 5, 40, "normal")
    )
      .to.emit(spaceDefender, "NewHighScore")
  ...
  it("Should track game statistics correctly", async function () {
    ...
    const gameStats = await spaceDefender.getGameStats();
  ...
  it("Should return correct player rank", async function () {
    ...
    const player1Rank = await spaceDefender.getPlayerRank(player1.address);
    const player2Rank = await spaceDefender.getPlayerRank(player2.address);
  ...
  it("Should have initialized achievements", async function () {
    const totalAchievements = await spaceDefender.getTotalAchievements();
    expect(totalAchievements.toNumber()).to.be.greaterThan(0);

    const achievements = await spaceDefender.getAllAchievements();
